const aboutPanel = {
    title: "About Me",
    description: [
        "I'm a NSCC graduate from the IT-Generalist program, so I have a wide-range of fundamental skills. However I have a passion for building things, whether that be web apps, a containerized service utilizing docker, a simulated network using GNS3. I came into programming mainly due to curiosity but it very quickly turned into something I loved. The process of creating, trouble-shooting, and then improving an app or service is something I am drawn to.", 
        "I love working across the stack, however I feel most comfortable on the front-end. Outside of web-development, I like experimenting with network topologies within GNS3, which has really helped me understand that, infrastructure really is the most important piece of the puzzle. Beyond all of this however, I am always looking to deepen my understanding, pick up new tools and things to experiment with, and build projects that have a use to someone.",
    ],
    // Turn skills into an object with three arrays, languages -- Tools -- Other
    skills: {
        Languages: [
            "Javascript",
            "Tailwind CSS",
            "PostgreSQL",
            "Python",
            "Django"
        ],
        Tools: [
            "Azure DevOps",
            "Github",
            "Docker",
            "GNS3",
            "Git"
        ],
        Other: [
            "Windows Security Fundamentals",
            "Networking Fundamentals",
            "Database Fundamentals",
            "Linux CLI"
        ]
    }
};

export default aboutPanel;